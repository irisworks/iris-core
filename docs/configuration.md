...
`read` tool then extracts its text layer via `pdftotext` (from `poppler-utils`,
installed by `bootstrap.sh`) rather than handing the model raw binary. A
scanned or image-only PDF has no text layer and reads back empty — there's no
OCR fallback, so a photo of a document should be sent as an image instead.
