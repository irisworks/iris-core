...
| `IRIS_WEBUI_PORT` / `IRIS_WEBUI_HOST` | — / `127.0.0.1` | Presence of `IRIS_WEBUI_PORT` enables the built-in web chat transport; `IRIS_WEBUI_HOST` widens the bind |
| `IRIS_WEBUI_PASSWORD` | — | Shared-secret login for the web UI. Unset = no auth gate (fine for loopback-only use; set before widening `IRIS_WEBUI_HOST` or exposing via `serve-public`) |
...
| `IRIS_TASK_MAX_CONCURRENT` | `3` | Cap on `task` runs in flight at once, process-wide across every channel and scheduled `--as-task` event; a call past the cap fails immediately with a `task failed: ...` tool error instead of queuing |
