...
| A photo Iris was sent never comes up, and she has nothing to say about it | The active model's entry in `models.json` declares `"input": ["text"]` — `pi-ai` strips image content for any such model before the request goes out | Add `"image"` to that model's `input` array if it's actually multimodal (current `data/models.json.template` ships `Kimi-K2.5`/`Kimi-K2.6` with `["text", "image"]`). Until then, Iris now surfaces this honestly instead of silently dropping it: check `journalctl -u iris` for `Dropping N image attachment(s)` and `last_prompt.jsonl` for a `<dropped_image_attachments>` block |
| Iris says a just-sent attachment is unavailable, or asks you to resend it | The Slack file hadn't finished downloading (or failed to) by the time the message was dispatched — `last_prompt.jsonl` will show an `<unavailable_attachments>` block with a `(still downloading — try again shortly)` or `(download failed)` note per path | Usually transient — Slack was slow to serve the file; ask Iris to look again a moment later. If it's consistently `(download failed)`, check `journalctl -u iris` for `Failed to download attachment` (bad token scope, deleted file, etc.). The bound-wait before dispatch defaults to 10s; raise `IRIS_ATTACHMENT_DOWNLOAD_TIMEOUT_MS` if attachments are routinely larger/slower than that |
| `read` on a PDF returns empty, or errors `pdftotext: command not found` | Empty text with no error means the PDF has no text layer (it's scanned/image-only — `pdftotext` has nothing to extract and there's no OCR fallback); the explicit error means the `pdf-text` [read handler](read-handlers.md)'s command isn't on `PATH` | For a scanned document, send it as an image instead so the vision model can read it. For the missing-binary case, install `poppler-utils` (`bootstrap.sh` does this automatically on fresh installs) |
...
| LLM calls fail (or compaction silently stops running) on a `models.json` provider whose `"apiKey"` is a `$`-prefixed env var reference, once that var is scrubbed from `process.env` under `store`/`proxy` mode | `pi-coding-agent`'s `AgentSession` calls `ModelRegistry.getApiKeyAndHeaders()` directly for compaction and branch-summary, bypassing the `getApiKey()` callback that falls back to the secret store | Fixed in current `iris-runtime` — the secret-store fallback now wraps `ModelRegistry.getApiKeyAndHeaders()` itself, so every caller (ours and `pi-coding-agent`'s internal ones) gets it. Update and restart `iris` |
| Compaction never runs (`journalctl -u iris` shows a 401 from the provider around the compaction threshold, or the session just keeps growing until the main call fails on length) on a `models.json` provider with `"apiKey"` set to a bare env var name (no `$` prefix) | A separate instance of the echo bug above: `ModelRegistry.getApiKeyAndHeaders()` itself echoes an unresolved bare apiKey value, and compaction/branch-summary call it directly instead of going through the `getApiKey()` guard normal turns use | Fixed in current `iris-runtime` — bare legacy-style `apiKey` values in `models.json` are normalized to `$NAME` before the registry loads them, so resolution is correct for every caller, not just normal turns. Update and restart `iris` |
| Every turn fails immediately (no network call, ~200ms) with `No API key found for <provider>`, on a `models.json` provider whose `"apiKey"` is a `$`-prefixed env var reference, once that var is scrubbed from `process.env` under `store`/`proxy` mode | `ModelRegistry.hasConfiguredAuth()` is a separate, synchronous method with the same `process.env`-only bypass as the row above — and it runs in `AgentSession`'s message-send pre-flight check, *before* `getApiKeyAndHeaders()`, so it throws before that fallback is ever reached | Fixed in current `iris-runtime` — the secret-store fallback now wraps `hasConfiguredAuth()` too, checking the store/env synchronously. Update and restart `iris` |
