...
| `read` on a PDF returns empty, or errors "is poppler-utils installed?" | Empty text with no error means the PDF has no text layer (it's scanned/image-only — `pdftotext` has nothing to extract and there's no OCR fallback); the explicit error means `pdftotext` isn't on `PATH` | For a scanned document, send it as an image instead so the vision model can read it. For the missing-binary case, install `poppler-utils` (`bootstrap.sh` does this automatically on fresh installs) |
