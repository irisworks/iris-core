...
| After `/reset`/`/clear`, replies still reference the old conversation (visible in `last_prompt.jsonl`) | Pre-current-fix versions replayed the channel's entire `log.jsonl` (the permanent, never-pruned record) back into the freshly-cleared context on the next message, since the sync step that catches up on offline messages couldn't tell a reset apart from a gap | Fixed in current `iris-runtime` — `reset()` now writes a `.reset-watermark` per channel and the log sync skips everything at/before it. Update and restart `iris` |
