...
- `GET /secrets/:name` caller identity is now derived from *which token authenticated* the request, not from the self-reported `X-Iris-Caller` header (IRIS-120, follow-up to IRIS-111). `terraform/modules/agent` provisions a unique `IRIS_API_TOKEN` per agent container (module output `api_token`, overriding the shared `.env` token for that container); register the value as a new `token` field on the agent's `agents.json` entry so the API can match it. Agent entries without a `token` still authenticate with the shared `IRIS_API_TOKEN` and resolve as unrestricted `iris`, same as before — existing installs keep working, but the allow-list only becomes a real security boundary (rather than hygiene/audit-trail) once each agent that needs it gets its own `token`. The `get-secret` skill no longer sends `X-Iris-Caller` (the server never reads it).
