...
- Langfuse tracing, session-correlated (#133). Each turn emits one trace tagged
  ...
  affect a run. `POST /sessions/:id/message` now also returns `sessionId`. See
...
- `buildSystemPrompt()` (`agent.ts`) told the agent its channel directory was `${workspacePath}/${channelId}` — omitting the `telegram/`/`slack/` transport subdir that `resolveChannelDir`/`resolveChannelPath` (`store.ts`) actually use on disk (e.g. real path `workingDir/telegram/tg-12345`, documented path `workingDir/tg-12345`). Both the "Workspace Layout" tree and the "Store skills in `${channelPath}/skills/`" instruction used this wrong path, so an agent creating files relative to its documented channel dir (e.g. `mkdir scratch`) wrote them to a bogus flat directory alongside — not inside — the real per-transport channel dir. The Docker-mode `translateToHostPath()` container-path prefix (`/workspace/${channelId}/`) had the same bug, since the whole `workingDir` is bind-mounted to `/workspace` (not just the one channel). Both now build the channel path via `resolveChannelPath(channelId)`.
- `workingDir/data/` — the metadata subfolder holding `sessions.json`, `mcp.json`, `channels.json`, and `telegram-owner.json` — stuttered into `<install>/data/data/...` on every standard install, since `workingDir` itself is conventionally named `data` (e.g. `/iris/data`). Renamed the subfolder to `meta/` (`sessions.ts`, `mcp/config.ts`, `slack.ts`, `telegram-claim.ts`); `main.ts` migrates any existing `workingDir/data/` to `workingDir/meta/` automatically on startup if `meta/` doesn't already exist.
...
- `iris-broker`'s gateway service map (`BUNDLED_SERVICES`) now covers `deepseek` and `mistral` (fixed public API base URLs, `Authorization: Bearer` header) and `azure-foundry`, whose per-install account subdomain is read from a new `AZURE_FOUNDRY_ACCOUNT` broker env var — `bootstrap.sh` writes it into `/iris/broker/broker.env` when `IRIS_PROVIDER=azure-foundry`. Previously only `anthropic`/`openai` (plus `resend`/`github`/`slack`) were bundled, so proxy-mode (`IRIS_SECRETS_MODE=proxy`) installs on the three newer LLM providers had no gateway route and had to hand-write `services.json` themselves. `amazon-bedrock` is intentionally not bundled — it authenticates via AWS SigV4 request signing, which the header-injection gateway can't express; it still works under `store`/`proxy` mode for secret storage, just not through `/proxy/:service`.
- Telegram claim flow now renders the one-time claim token as a scannable QR code alongside the plaintext token — both in `bootstrap.sh`'s claim box (shelling out to the `qrcode-terminal` module already installed under `iris-runtime/node_modules`, resolving the bot's `@username` via `getMe`) and in the runtime's own `journalctl` output on every startup/restart. The QR code encodes a `t.me/<bot>?start=<token>` deep link rather than the bare token, so scanning it opens a chat with the bot's `/start <token>` message pre-filled — one tap on Send claims it, no typing or copy/paste required (the claim gate now unwraps the `/start <token>` command form in addition to accepting the bare token). An unclaimed bot also replies with a pointer to check the terminal for the claim token instead of silently ignoring any message that isn't a valid pending token (IRIS-92).

### Fixed

- `bootstrap.sh` re-runs without `--setup` and without an explicit `--keyvault`/`--no-keyvault` (e.g. `bootstrap.sh --secrets-mode=proxy`) fell through `NO_KEYVAULT`'s hardcoded `false` default into the Key Vault code path, forcing an unwanted `az login` on installs that never used Key Vault. Worse, that path's non-setup "restore mode" branch only restores `IRIS_PROVIDER`/`IRIS_MODEL`, not the LLM API key vars — so the real key got silently overwritten with an empty string in `/iris/.env` before a secrets-mode migration could pick it up, breaking the LLM call entirely with no error at prune time. Re-runs now infer `NO_KEYVAULT` from the existing install's `IRIS_KEY_VAULT` (empty → `/iris/.env` path, set → Key Vault path with that vault) whenever neither flag is passed explicitly, keeping re-runs on whichever path the install already uses.
- `get-secret`/`set-secret` are documented and used by several skills (`send-email`, `github`, `transcribe-audio`, etc.) as bare commands, but nothing put either on `PATH` — the sandbox executor spawns `sh -c "<cmd>"` with the plain inherited `PATH`, and only `iris-secret` was symlinked into `/usr/local/bin`. Every bare `get-secret NAME` call failed with `command not found`, relying on the model noticing the failure and falling back to the skill's absolute path (`/iris/data/skills/get-secret/get-secret`) instead — which works, per `pi-coding-agent`'s own skill-loading system prompt guidance, but adds a failed call and a recovery step to every single invocation. `bootstrap.sh` now symlinks both into `/usr/local/bin`, same as `iris-secret`, so the documented bare-command usage actually works.
- `bootstrap.sh` wrote `/iris/.env` via `sudo tee` (root-owned) and only `chmod 600`'d it afterward — never `chown`'d it back to `$TARGET_USER`. `iris.service` runs as `User=$TARGET_USER` and reads its config purely via `dotenv/config` at process startup (the unit has no `EnvironmentFile=`), so it silently couldn't read its own `.env` at all: `dotenv` swallows the read error, every setting falls back to its hardcoded default (`IRIS_PROVIDER`/`IRIS_MODEL` become `anthropic`/`claude-sonnet-4-5` regardless of what was configured), `IRIS_SECRET_BROKER_URL` is never set so the broker becomes unreachable, and Telegram/Slack tokens resolve to nothing — with no error message pointing at the actual cause. Manifested as the Telegram claim-token flow timing out with a misdiagnosed warning (blamed `/iris/data/data/` ownership, which was never the issue — Telegram's transport never even initialized). `bootstrap.sh` now `chown`s `/iris/.env` to `$TARGET_USER` right after writing it, and the claim-file timeout warning now points at `/iris/.env` ownership too.
- `getApiKey()` in `agent.ts` could send a `pi-coding-agent` internal placeholder string as the literal API key. `resolveConfigValue()` (vendored, in `resolve-config-value.js`) falls back to echoing its raw config string (e.g. `"MISTRAL_API_KEY"`) when the env var it names isn't set: `process.env[config] || config`. `models.json`'s custom providers (`azure-foundry`, `deepseek`, `mistral`, `custom`) configure `"apiKey"` as an env var name, so once `store`/`proxy` secrets mode scrubs that var from `process.env` after migrating the real key to the secret store, `getApiKeyAndHeaders()` reports `ok: true, apiKey: "MISTRAL_API_KEY"` (the literal env var name, not a real key), and the broker/env fallback below it never ran, since `auth.ok && auth.apiKey` was already satisfied by the garbage string. The request then went out with that literal string as the bearer token; the provider's API 401s fast, but the streaming client doesn't surface a non-200 response as an error, so the run hung indefinitely with nothing in the logs — the same class of symptom `eaf0648` investigated for the native Mistral provider, but a distinct cause hitting the already-fixed `openai-completions` path specifically under `store`/`proxy` mode. `getApiKey()` now reads `models.json`'s configured `"apiKey"` field itself and treats a returned `apiKey` that exactly matches that raw config string as unresolved, falling through to the broker/env lookups instead.

### Removed

- `bootstrap.sh` no longer runs an unconditional `gh auth login` device-code flow as step 3 of every install path, including the zero-cloud `--setup --no-keyvault` quickstart (IRIS-122). It wasn't referenced by the README quickstart, blocked the curl-pipe promise of `install.sh` with an interactive GitHub login most installs don't need, and the `GH_USER` it fetched via `gh api user` was never used downstream. GitHub access for Iris's self-extension/PR features is still covered by the existing optional "Add GitHub token for repo access?" prompt (`GITHUB_TOKEN`, fine-grained PAT), unchanged. `docs/SETUP.md` Option 1 step list updated to match.
...
- `bootstrap.sh`'s optional GitHub token prompt now also asks which repo Iris
  ...
  to the public repo.
- Telegram messages describing an attachment (e.g. "Here is the PDF: `[iris_note.pdf](iris_note.pdf)`") rendered the raw `[text](url)` bracket syntax literally instead of a link, since `toTelegramHtml()` only converted `**bold**`/`_italic_`/`` `code` `` and left markdown links untouched for `parse_mode: "HTML"`. The prompt already told the model not to use link syntax, but it did anyway (a strong LLM habit when referencing a file by name). `toTelegramHtml()` now handles `[text](url)` defensively: an `http(s)` URL becomes a real `<a href>`, anything else (almost always a bare filename — the actual file already went out separately via the `attach` tool) is reduced to just the label text so no broken bracket/paren syntax reaches the chat.
- Telegram replies with tool calls showed the final answer edited into the original "_Thinking..._" message, which stayed at the *top* of the exchange while tool-call updates (posted as separate messages via `respondInThread`) landed *below* it — so the final answer appeared before the steps that produced it, forcing a scroll back up to read it. `replaceMessage` in `createTelegramContext` (`src/transports/telegram/telegram.ts`) now checks whether any tool-call messages were posted (`extraMessageIds.length > 0`): if so, it deletes the "Thinking..." message and posts the final answer fresh at the bottom instead of editing in place, so messages read in chronological order. A simple exchange with no tool calls is unaffected — it still edits the single "Thinking..." message in place, so it doesn't turn into two messages for no reason.
...
- `bootstrap.sh`'s Telegram prompt now shows an in-terminal setup box (message `@BotFather`, `/newbot`, token format) before asking for the token, mirroring the existing Slack/GitHub setup boxes — previously it jumped straight to `prompt_secret` with no guidance, unlike every other integration. `prompt_secret` now trims leading/trailing whitespace (mobile paste from the Telegram app commonly drags in a stray newline that a masked `-s` prompt gives no way to see), and the Telegram flow extracts the `<numeric id>:<hash>` token via regex if stray text landed on the same line as the paste, echoing back a masked confirmation (`123456789…xyz9 (46 chars)`) so the operator has some feedback that the paste landed correctly before bootstrap continues. The box also now suggests doing the BotFather chat at `web.telegram.org` or the desktop app on the same machine as the terminal — Telegram syncs the chat to every logged-in device, so the token can be copy-pasted locally instead of moved off a phone.
