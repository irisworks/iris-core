...
- **MCP servers** — external toolsets (stdio or remote HTTP) plug in via `meta/mcp.json`, hot-reloaded and manageable through chat (see `docs/mcp.md`)
...
- **Encrypted secret store by default** — credentials live in an encrypted local store (`IRIS_SECRETS_MODE=store`), or a separate-uid injection proxy (`proxy`), with `env` kept as a legacy opt-out — plus one-time drop links so users never have to paste a secret into chat (see `docs/secrets.md`)
...
| `IRIS_SECRETS_MODE` | `store` | `store` (default) \| `proxy` \| `env` (legacy opt-out) — encrypted store / injection-proxy credential backend (see `docs/secrets.md`) |
